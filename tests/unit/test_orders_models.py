...
def test_find_email(new_order):
    ...
    assert isinstance(new_order.find_by_email("example@email.com"), str)
    assert not isinstance(new_order.find_by_email("example@email.com"), int)
    assert new_order.find_by_email("example@email.com") == "1"
    

def test_find_id(new_order):
    """
    ...
    """
    new_order.id = 42
    assert isinstance(new_order.find_by_id(42), str)
    assert not isinstance(new_order.find_by_id(42), int)
